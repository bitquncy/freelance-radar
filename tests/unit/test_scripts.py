...
def test_backup_restore_scripts_help_and_dry_run(tmp_path: Path) -> None:
    ...
    backup = _run([sys.executable, "scripts/backup_db.py", "--dry-run", "--output-dir", str(tmp_path)], env=base_env)
    assert backup.returncode == 0, backup.stdout + backup.stderr
    assert "would run pg_dump" in backup.stdout

    restore = _run([sys.executable, "scripts/restore_db.py", "backup.dump", "--dry-run"], env=base_env)
...
def test_owner_scripts_require_confirmation() -> None:
    ...
    export = _run([sys.executable, "scripts/export_owner_data.py", "--user-id", "1"], env=env)
    assert export.returncode != 0
    assert "EXPORT-OWNER" in export.stderr

    delete = _run([sys.executable, "scripts/delete_owner_data.py", "--user-id", "1"], env=env)
