...
- Поддерживаются медиагруппы, URL-кнопки и фильтры по типу чата/языку
- Отложенный запуск, прогресс, пауза, возобновление и остановка
- PostgreSQL-очередь с отдельным статусом каждого получателя и восстановлением без дублей
- Redis sliding-window limiter, глобальный PTB limiter и часовой кулдаун одного чата
- FSM хранится в Redis и закрывает черновик после 15 минут бездействия
...
| `REDIS_URL` | — | Redis для FSM и общего rate limit; обязателен в production |
